...
app.post("/login", async (req, res) => {
  try {
    ...
    !user
      ? res.status(422).json({
          success: false,
          msg: "User doesn't exist, Please enter valid email and password",
        })
      : res.status(200).json({
          success: true,
          msg: "User found",
    ...
    res.status(500).json({
      success: false,
      msg: "Couldn't validate user somthing went wrong",
    });
  }
});
...
app.post("/getcards", async (req, res) => {
  try {
    ...
    const data = await Card.find({ userId }).select({
      date: 1,
      heading: 1,
      note: 1,
      _id: 0,
...
app.post("/getuser", async (req, res) => {
  try {
    ...
    !user
      ...
      : res
          .status(200)
          .json({ success: true, msg: "User found", username: user.username });
  } catch {
...
app.post("/addfriend", async (req, res) => {
  try {
    ...
    !user
      ...
      : res
          .status(200)
          .json({ success: true, msg: "Friend added succesfully", user });
  } catch (err) {
...
app.post("/loadfriends", async (req, res) => {
  try {
    ...
    !user
      ...
      : res
          .status(200)
          .json({ success: true, msg: "Friends list found", friends: user });
  } catch (err) {
